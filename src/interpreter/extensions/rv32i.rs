use crate::interpreter::{
    mmu::Mmu,
    riscv_core::{BInstruction, Exception, IInstruction, JInstruction, RInstruction, RVCore, SInstruction, UInstruction},
};
...
pub fn addi(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn xori(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn ori(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn andi(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn slli(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn srli(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn srai(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn slti(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn sltui(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn lb(instr: &IInstruction, mmu: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
    let addres = rs1_val.wrapping_add(instr.imm);
    let mem_val = mmu.read_byte(addres as usize)?;
...
pub fn lh(instr: &IInstruction, mmu: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
    let addres = rs1_val.wrapping_add(instr.imm);
    ...
    let val_1 = mmu.read_byte(addres as usize)?;
    let val_2 = mmu.read_byte(addres.wrapping_add(1) as usize)?;
...
pub fn lw(instr: &IInstruction, mmu: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
    let addres = rs1_val.wrapping_add(instr.imm);

    let val_1 = mmu.read_byte(addres as usize)?;
    let val_2 = mmu.read_byte(addres.wrapping_add(1) as usize)?;
    let val_3 = mmu.read_byte(addres.wrapping_add(2) as usize)?;
    let val_4 = mmu.read_byte(addres.wrapping_add(3) as usize)?;
...
pub fn lbu(instr: &IInstruction, mmu: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
    let addres = rs1_val.wrapping_add(instr.imm);
    
    let val = mmu.read_byte(addres as usize)? as u32;
    core.write_reg(instr.rd, val);
    
    Ok(())
}

pub fn lhu(instr: &IInstruction, mmu: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
    let addres = rs1_val.wrapping_add(instr.imm);
...
pub fn sb(instr: &SInstruction, mmu: &mut Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
    let rs2_val = core.read_reg(instr.rs2);
    let addres = rs1_val.wrapping_add(instr.imm);

    mmu.write_byte(addres as usize, rs2_val as u8)
}

pub fn sh(instr: &SInstruction, mmu: &mut Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
    let rs2_val = core.read_reg(instr.rs2);
    let addres = rs1_val.wrapping_add(instr.imm);
    ...
    mmu.write_byte(addres as usize, rs2_val as u8)?;
    mmu.write_byte(addres.wrapping_add(1) as usize, (instr.rs2 >> 8) as u8)
}

pub fn sw(instr: &SInstruction, mmu: &mut Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
    let rs2_val = core.read_reg(instr.rs2);
    let addres = rs1_val.wrapping_add(instr.imm);

    mmu.write_byte(addres as usize, rs2_val as u8)?;
    mmu.write_byte(addres.wrapping_add(1) as usize, (rs2_val >> 8) as u8)?;
    mmu.write_byte(addres.wrapping_add(2) as usize, (rs2_val >> 16) as u8)?;
    mmu.write_byte(addres.wrapping_add(3) as usize, (rs2_val >> 24) as u8)
}
...
pub fn jarl(instr: &IInstruction, _: &Mmu, core: &mut RVCore) -> Result<(), Exception> {
    let rs1_val = core.read_reg(instr.rs1);
...
pub fn fence(_: &IInstruction, _: &Mmu, _core: &mut RVCore) -> Result<(), Exception> {
    // TODO
    Ok(())
}